...
import { defaultOutLinkForm, defaultThirdPartyOutLinkForm } from '@/web/core/app/constants';
import type { OutLinkEditType, OutLinkSchema } from '@fastgpt/global/support/outLink/type.d';
import {
  ThirdPartyAuthEnum,
  type PublishChannelEnum
} from '@fastgpt/global/support/outLink/constant';
...
const SelectUsingWayModal = dynamic(() => import('../Link/SelectUsingWayModal'));
...
const ThirdPartyLink = ({
  ...
  } = useQuery(['initThirdPartyShareChatList', appId], () =>
    getShareChatList({ appId, type: publishType })
  );
      ...
      <Flex justifyContent={'space-between'}>
        <HStack>
          <Box color={'myGray.900'} fontSize={'lg'}>
            {t('common:core.app.Share third party')}
          </Box>
        ...
        <Button
          ...
          {...(shareChatList.length >= 10
            ...
            : {})}
          onClick={() => setEditLinkData(defaultThirdPartyOutLinkForm)}
        >
      ...
      <TableContainer mt={3}>
        <Table variant={'simple'} w={'100%'} overflowX={'auto'} fontSize={'sm'}>
          <Thead>
            <Tr>
              <Th>{t('common:Name')}</Th>
              <Th>{t('common:core.app.Share third party type')}</Th>
              {feConfigs?.isPlus && (
                <>
          ...
          <Tbody>
            {shareChatList.map((item) => (
              <Tr key={item._id}>
                <Td>{item.name}</Td>
                <Td>{item.thirdPartyAuth?.authType || 'NONE'}</Td>
                {feConfigs?.isPlus && (
                  <>
                    <Td>
                        ...
                        children: [
                          {
                            ...
                            onClick: () =>
                              setEditLinkData({
                                ...
                                thirdPartyAuth:
                                  item.thirdPartyAuth ?? defaultOutLinkForm.thirdPartyAuth,
                                limit: item.limit
                              })
                          },
                          {
...
function EditLinkModal({
  ...
  const thirdPartyAuthType: ThirdPartyAuthEnum = watch('thirdPartyAuth.authType');
    ...
    >
      ...
      >
        <Box pr={[0, 4]} flex={1} borderRight={['0px', '1px']} borderColor={['', 'myGray.150']}>
          ...
          <Flex alignItems={'center'} mt={4}>
            <FormLabel flex={'0 0 150px'}>{t('common:core.app.Share third party type')}</FormLabel>
            <MySelect<ThirdPartyAuthEnum>
              value={thirdPartyAuthType}
              list={Object.values(ThirdPartyAuthEnum).map((item) => {
                return {
                  label: item,
                  value: item as ThirdPartyAuthEnum
                };
              })}
              onChange={(val) => {
                setValue('thirdPartyAuth.authType', val as ThirdPartyAuthEnum);
              }}
            ></MySelect>
          </Flex>
          {feConfigs?.isPlus && (
...
export default React.memo(ThirdPartyLink);
